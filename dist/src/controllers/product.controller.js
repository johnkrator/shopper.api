...
const addProduct = (0, asyncHandler_1.default)(async (req, res) => {
    try {
        ...
        let categoryObj = await category_model_1.default.findOne({ name: category });
        if (!categoryObj) {
            categoryObj = new category_model_1.default({ name: category });
            await categoryObj.save();
        }
        // Upload the image to Cloudinary and get the URL
        const cloudinaryResult = await (0, upload_controller_1.uploadImageToCloudinary)(image);
...
* */
const fetchProducts = (0, asyncHandler_1.default)(async (req, res) => {
    ...
    const count = await product_model_1.default.countDocuments({ ...keyword });
    const totalProducts = await product_model_1.default.countDocuments({});
    const products = await product_model_1.default.find({ ...keyword })
        .skip(startIndex)
        .limit(limit);
...
* */
const fetchAllProducts = (0, asyncHandler_1.default)(async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const startIndex = (page - 1) * limit;
    try {
...
const removeProduct = (0, asyncHandler_1.default)(async (req, res) => {
    try {
        const productId = req.params.id;
        const product = await product_model_1.default.findById(productId);
        if (!product) {
...
const fetchTopProducts = (0, asyncHandler_1.default)(async (req, res) => {
    const page = parseInt(req.query.page) || 1;
...
* */
const fetchNewProducts = (0, asyncHandler_1.default)(async (req, res) => {
    const page = parseInt(req.query.page) || 1;
...
* */
const filterProducts = (0, asyncHandler_1.default)(async (req, res) => {
    try {
...
const addProductReview = (0, asyncHandler_1.default)(async (req, res) => {
    try {
